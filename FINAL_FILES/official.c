...
void simulated_annealing(){
    ...
    for(;;){
        ...
        if(new_cost<=best&&worst_time<15&&flag==0){
            best=new_cost;
            for (int i = 0; i < passenger_count; i++) {     
                route[i] = new[i];     
            }
...
void fix_route(){
    int check=0;
    int temp=0;
    double worst_marker=worst_time;
    ...
    while(worst_marker>5){
        ...
        for(int i=0; i<passenger_count; i++){
            ...
            if(initial_diff[check]>5){
                ...
                if(worst_time<worst_marker&&flag==0){
                    worst_marker=worst_time;
                    for(int i=0; i<passenger_count; i++){
                        route[i]=test[i];
                    }
                    printf("WORST=%f\n\n", worst_marker);
    ...
    for(int i=0; i<route_stops-1; i++){
        if(input[route[i]]==input[route[i+1]]&&fabs(acctime[i+1]-acctime[i])<5){
            ...
            for(int j=i+1; j<route_stops-1; j++){
                route[j]=route[j+1];
                acctime[j]=acctime[j+1];
            }
            route_stops--;
        }
    }
...
int main(void){
    ...
    FILE *fptr;
    if ((fptr = fopen("/Users/satokamimura/Desktop/seniorproject/30psg.txt","r")) == NULL){
    ...
    fscanf(fptr, "%d ", &final_count);


    for(i=6; i<=final_count*2; i+=6){

        passenger_count=i;

        for(int j=i-6; j<passenger_count; j+=2){
        ...
        printf("input ");
        for(int j=i-6; j<passenger_count; j++){
            printf("%d ", input[j]);
        ...
        count_patrols(greedy_route, passenger_count);
