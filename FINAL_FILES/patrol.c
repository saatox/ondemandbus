...
double calc_time(int new_route[]){
    ...
    for (int i=1; i<passenger_count; i++){
        ...
        if((acctime[end]-rqtime[end])>=worst_time){
            worst_time=acctime[end]-rqtime[end];
        }
    ...
    for(int i=0; i<passenger_count-1; i+=2){
        ...
        if(current-best>worst_rt){
            worst_rt=current-best;
        }
        total_rt+=current-best;
    }
    ptr=count_patrols(new_route, passenger_count);
    //printf("\n");
    return total;
}
...
void simulated_annealing(){
    ...
    if(worst_time>initial_worst+20){
        printf("REDO, worst=%f\n", worst_time);
        simulated_annealing();
    }
    else if(worst_time>initial_worst){
        for (int i = 0; i < passenger_count; i++) {     
...
int add_patrol_time(){
    double to=0, back=0;
    int start=0, next=0;
    int patrol_route_count=route_stops;
    int patrol_route[MAX];
    ...
    for(int i=0; i<passenger_count-1; i++){
        ...
        for(j=0; j<bus_stops; j++){
            ...
            if(acctime[next]-acctime[start]>(to+back+2)&&potential!=input[next]&&potential!=input[start]){
                ...
                for (m = patrol_route_count-1; m>i+(patrol_route_count-route_stops); m--){
...
void fix_route(){
    ...
    route_stops=passenger_count;

    printf("\n");
    for(int i=0; i<route_stops-1; i++){
    ...
    count_patrols(route, route_stops);
...
void best_time_comp(){
    ...
    for(int i=0; i<passenger_count-1; i+=2){
        ...
        if((current-best)>0.00000001){
            printf("from %d to %d: best=%f, current=%f-%f=%f, request=%f-%f, overtime=%f\n", start, next, best, acctime[route[i+1]], acctime[route[i]], current, rqtime[i+1], rqtime[i], current-best);
        }
...
int main(void){
    ...
    FILE *fptr;
    if ((fptr = fopen("/Users/satokamimura/Desktop/seniorproject/performanceEval/patrol/test10.txt","r")) == NULL){
   ...
   clock_t t;
    ...
    FILE *fp;
    fp = fopen("patrol.txt", "a");
    fprintf(fp, "%f\t%f\t%f\t%f\t%d\t%f\t%f\t%f\t%f\t%d\n", worst_time, calc_time(route), total_rt, worst_rt, final_ptr, initial_worst, initial_cost, initial_rt, initial_worstrt, initial_ptr);
